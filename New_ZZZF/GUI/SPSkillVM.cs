...
namespace New_ZZZF
{
    public class SPSkillVM : ViewModel, IInventoryStateHandler
    {
        ...
        public bool IsItemEquipmentPossible(SPSkillItemVM itemVM)
        {
            ...
            SkillFactory._skillRegistry.TryGetValue(itemVM.ItemRosterElement.EquipmentElement.Item.Name.Value, out var skill);
            if (skill == null)
            {
                return true; 
            }
            ...
            switch (this.TargetEquipmentType)
            {
                ...
                case EquipmentIndex.Weapon3:
                    if (skill.Type == SkillType.CombatArt || skill.Type == SkillType.Spell || skill.Type == SkillType.Passive)
                    { CanUseFlag2 = true; }
                    break;
                case EquipmentIndex.Gloves:
                    if (skill.Type == SkillType.CombatArt )
                    { CanUseFlag2 = true; }
                    break;
        ...
        private void EquipEquipment(SPSkillItemVM itemVM)
        {
            ...
            // 获取目标装备槽位的当前物品
            SPSkillItemVM itemFromIndex = this.GetItemFromIndex(this.TargetEquipmentType);

            // 如果目标槽位已有相同物品和修饰符，则无需重复装备
            if (itemFromIndex != null
                && itemFromIndex.ItemRosterElement.EquipmentElement.Item == SPSkillItemVM.ItemRosterElement.EquipmentElement.Item
                && itemFromIndex.ItemRosterElement.EquipmentElement.ItemModifier == SPSkillItemVM.ItemRosterElement.EquipmentElement.ItemModifier)
            {
                return;
            }
            ...
            if (!flag)
            {
                ...
                // 如果当前物品是盾牌且不在装备栏中，则寻找已装备的盾牌槽位
                if (itemVM.ItemRosterElement.EquipmentElement.Item.Type == ItemObject.ItemTypeEnum.Shield
                    && itemVM.InventorySide != InventoryLogic.InventorySide.Equipment)
                {
                    // 遍历所有武器槽位，查找已装备的盾牌
                    for (EquipmentIndex equipmentIndex2 = EquipmentIndex.WeaponItemBeginSlot;
                         equipmentIndex2 <= EquipmentIndex.NumAllWeaponSlots;
                         equipmentIndex2++)
                    {
                        SPSkillItemVM itemFromIndex2 = this.GetItemFromIndex(equipmentIndex2);
                        bool flag2;
                        ...
                        else
                        {
                            ...
                            flag2 = (itemType == ItemObject.ItemTypeEnum.Shield);
                        }

                        // 如果找到盾牌槽位，则记录该槽位索引并退出循环
                        if (flag2)
                        {
            ...
            // 创建装备转移命令（从当前物品位置转移到装备栏）
            TransferCommand item3 = TransferCommand.Transfer(
                1, // 数量为1
                itemVM.InventorySide, // 来源库存位置（如玩家库存、对方库存等）
                InventoryLogic.InventorySide.Equipment, // 目标位置为装备栏
                SPSkillItemVM.ItemRosterElement, // 要转移的物品元素
                SPSkillItemVM.ItemType, // 物品类型
                this.TargetEquipmentType, // 目标装备槽位索引
                this._currentCharacter, // 当前角色
                !this.IsInWarSet // 是否为平民装备模式
            );
            list.Add(item3); // 添加到命令列表

            // 如果需要替换装备（flag为true），则创建额外的转移命令
            if (flag)
            {
                TransferCommand item4 = TransferCommand.Transfer(
                    1, // 数量为1
                    InventoryLogic.InventorySide.PlayerInventory, // 来源为玩家库存
                    InventoryLogic.InventorySide.Equipment, // 目标为装备栏
                    itemFromIndex.ItemRosterElement, // 原装备的物品元素
                    EquipmentIndex.None, // 原装备槽位（无）
                    SPSkillItemVM.ItemType, // 新物品类型
                    this._currentCharacter, // 当前角色
                    !this.IsInWarSet // 是否为平民装备模式
                );
                list.Add(item4); // 添加替换命令
            }
