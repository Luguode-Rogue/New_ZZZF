...
namespace New_ZZZF
{
    ...
    public class AgentSkillComponent : AgentComponent
    {
        ...
        //------------------------ 资源与状态 ------------------------
        private float _currentMana = 100f;       // 当前法力值
        private float _currentStamina = 100f;    // 当前耐力值
        private float _globalCooldownTimer = 0f; // 公共CD计时器（仅法术）
        private bool _isInCombatArtState;        // 是否处于战技准备状态

        // 冷却计时器（Key: 技能实例, Value: 剩余冷却时间）
        private readonly Dictionary<SkillBase, float> _cooldownTimers = new Dictionary<SkillBase, float>();
        ...
        private void TryActivateSkill(SkillBase skill)
        {
            ...
            if (skill.Activate(Agent))
            {
                ...
                {
                    //Agent.SetActionChannel(1, ActionIndexCache.Create("act_horse_command_follow"), false, 172UL, 1, 1.5f, -0.2f, 0.4f, 0.5f);
                    _currentMana = Math.Max(0, _currentMana - skill.ResourceCost);
                }
                else if (skill.Type == SkillType.CombatArt)
                {
                    //Agent.SetActionChannel(1, ActionIndexCache.Create("act_horse_command_follow"), false, 172UL, 1, 1.5f, -0.2f, 0.4f, 0.5f);
                    _currentMana = Math.Max(0, _currentMana - skill.ResourceCost);
                }
        ...
        private bool CanActivateSkill(SkillBase skill)
        {
            ...
            // 资源检查
            bool hasResource = (skill.Type == SkillType.Spell || skill.Type == SkillType.CombatArt) ?
