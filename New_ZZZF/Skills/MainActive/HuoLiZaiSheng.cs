...
namespace New_ZZZF
{
    internal class HuoLiZaiSheng : SkillBase
    {
        ...
        {
            SkillID = "HuoLiZaiSheng";      // 必须唯一
            Type = SkillType.MainActive;    // 类型必须明确
            Cooldown = 2;             // 冷却时间（秒）
            ResourceCost = 0f;        // 消耗
            Text = new TaleWorlds.Localization.TextObject("{=ZZZF0058}HuoLiZaiSheng");
            Difficulty = null;// new List<SkillDifficulty> { new SkillDifficulty(50, "跑动"), new SkillDifficulty(5, "耐力") };//技能装备的需求
            Description = new TaleWorlds.Localization.TextObject("{=ZZZF0059}群体状态，使用后附近士兵获得无尽的活力，每秒回复所有已损失的生命值。持续时间：30秒。冷却时间：40秒。");
        }
        public override bool Activate(Agent agent)
        {
            ...
            if (values!=null&&values.Count>0)
            {
                ...
                {
                    // 每次创建新的状态实例
                    List<AgentBuff> newStates = new List<AgentBuff> { new HuoLiZaiShengBuff(30f, agent), }; // 新实例
