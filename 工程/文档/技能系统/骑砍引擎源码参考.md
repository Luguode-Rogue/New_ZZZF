> **概要**：本文档收录骑砍2引擎中有价值的源码片段，包括焦点交互、视线检测、自动格挡、武器丢弃、上下马、属性计算、弹药管理、骨骼碰撞等核心功能的实现代码。适合需要理解引擎底层机制或参考类似实现时使用。
...
public void HandleDropWeapon(bool isDefendPressed, EquipmentIndex forcedSlotIndexToDropWeaponFrom)//处理角色（Agent）丢弃武器的逻辑
{
    ...
    if (this.State == AgentState.Active && currentActionType != Agent.ActionCodeType.ReleaseMelee && currentActionType != Agent.ActionCodeType.ReleaseRanged && currentActionType != Agent.ActionCodeType.ReleaseThrowing && currentActionType != Agent.ActionCodeType.WeaponBash)
    {
        ...
        {
            // 获取主手和副手当前持有的武器槽位索引
            EquipmentIndex wieldedItemIndex = this.GetPrimaryWieldedItemIndex(Agent.HandIndex.MainHand);
            EquipmentIndex wieldedItemIndex2 = this.GetPrimaryWieldedItemIndex(Agent.HandIndex.OffHand);
	...
	//里面有人物的加速度属性的设置
        ...
        private void UpdateHorseStats(Agent agent, AgentDrivenProperties agentDrivenProperties)
		{
            ...
            //可以通过这个语句，来获取到武器吃的技能加成
                        ...
                        EquipmentIndex mainHandIndex = agent.GetPrimaryWieldedItemIndex(Agent.HandIndex.MainHand);
...
//盾牌耐久条的显示，改一下做成体力魔法条
		...
		public void OnMainAgentWeaponChange()
		{
			...
			EquipmentIndex wieldedItemIndex = this._mission.MainAgent.GetPrimaryWieldedItemIndex(Agent.HandIndex.OffHand);
			...
			wieldedItemIndex = this._mission.MainAgent.GetPrimaryWieldedItemIndex(Agent.HandIndex.MainHand);
			if (wieldedItemIndex > EquipmentIndex.None && wieldedItemIndex < EquipmentIndex.NumAllWeaponSlots)
			{
