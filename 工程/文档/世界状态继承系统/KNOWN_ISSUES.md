...
| KNW-06 | 英雄模板复刻为"重建相似游荡英雄"，非原对象复用 — 跨存档 `StringId` 不稳定，无法找回原英雄 | 中 | 已知 | 预期行为：新档出现的是属性相似的游荡英雄，姓名相同但为独立实体 |
| KNW-07 | 复刻英雄不还原装备 / 部队 / 家族从属关系，仅还原姓名、文化、等级、技能、特性、职业、性别、体型 | 低 | 已知 | 设计范围仅限"游荡英雄模板"，避免政治格局污染 |
| KNW-08 | `ResurrectedHeroTracker` 登记表为进程内存储，游戏重启后清空，验证按钮仅当前会话有效 | 低 | 已知 | 登记表不参与存档，仅用于开发期快速验证 |
