...
{
    /// <summary>
    /// REV10 native battle-scene photo capture.
    ///
    /// Uses Bannerlord's dedicated ThumbnailCreatorView pipeline instead of attaching a
    /// continuously rendering SceneView directly to the live Mission.Scene. This keeps the
    /// off-screen render request isolated from MissionScreen's normal input/raycast path.
    ///
    /// Rendering path:
    ///   Mission.Scene + Camera
    ///       -> ThumbnailRenderRequest
    ///       -> ThumbnailCreatorView
    ///       -> completed RenderTarget Texture
    ///       -> Texture.SaveToFile
    ///       -> CPU validation
    ///       -> TerrainCache.ApplyPhotoPixels
    /// </summary>
    public sealed class TerrainPhotographerRev10
    {
        public static readonly TerrainPhotographerRev10 Instance = new TerrainPhotographerRev10();

        private const int Revision = 10;
        ...
        private TerrainPhotographerRev10() { }
        ...
        public void Start(Mission mission, TerrainCache cache)
        {
            ...
            try
            {
                ...
                TacticalMapLog.Info("[PhotoNative] REV=" + Revision +
                    ...
                    " target=" + GetTargetWidth() + "x" + GetTargetHeight() +
                    " queued=true");
            }
            catch (Exception ex)
            {
        ...
        public void OnMissionEnd()
        {
            ...
            TacticalMapLog.Info("[PhotoNative] REV=" + Revision +
                " mission end; thumbnail renderer retained idle.");
        }
        ...
        private void QueueRenderRequest()
        {
            if (_mission == null || _mission.Scene == null)
                throw new InvalidOperationException("Mission.Scene is unavailable.");

            try
            {
                _thumbnailView.ClearRequests();
            }
            catch
            {
                // ClearRequests is best-effort; a new request is still valid below.
            }
        ...
        private bool TickWaitingRender()
        {
            ...
            TacticalMapLog.Info("[PhotoNative] REV=" + Revision +
                " render callback received; saving completed texture file=" + _savePath);
        ...
        private static void OnThumbnailRenderComplete(string renderId, Texture renderTarget)
        {
            ...
            TacticalMapLog.Info("[PhotoNative] REV=" + Revision +
                " thumbnail callback renderId=" + renderId);
        }
        ...
        {
            _failed = true;
            TacticalMapLog.Error("[PhotoNative] REV=" + Revision + " FAILED: " + reason);
            CancelActiveRequest();
            CleanupFileOnly();
            _stage = Stage.Idle;
            return false;
