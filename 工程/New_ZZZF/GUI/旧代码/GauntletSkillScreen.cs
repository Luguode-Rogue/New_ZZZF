    ...
    public class GauntletSkillScreen : ScreenBase, IInventoryStateHandler, IGameStateListener, IChangeableScreen
    {
        ...
        protected override void OnFrameTick(float dt) // 每帧更新逻辑
        {
            ...
            {           
                ...
                if (this._gauntletLayer.Input.IsHotKeyReleased("Exit") || this._gauntletLayer.Input.IsGameKeyReleased(38)) // 如果退出键按下并释放
                {
                    this.ExecuteCancel(); // 执行取消操作
                    return;
                }
                if (this._gauntletLayer.Input.IsHotKeyReleased("Confirm")) // 如果确认键按下并释放
                {
                    this.ExecuteConfirm(); // 执行确认操作
                    return;
                }
                if (this._gauntletLayer.Input.IsHotKeyReleased("Reset")) // 如果重置键按下并释放
                {
        ...
        protected override void OnInitialize() // 初始化逻辑
        {
            ...
            // Sprite category loading handled automatically in newer versions
            ...
            this._gauntletLayer = new GauntletLayer("GauntletLayer", 15,  true) // 创建 Gauntlet 层
            {
        ...
        // FilterInventoryAtOpening removed from IInventoryStateHandler in new API; kept as public method
        public void FilterInventoryAtOpening(InventoryScreenHelper.InventoryCategoryType inventoryCategoryType) // 在打开时过滤库存
        {
            ...
            {
                case InventoryScreenHelper.InventoryCategoryType.Armors: // 如果是盔甲类别
                    this._dataSource.ExecuteFilterArmors(); // 执行盔甲过滤
                    return;
                case InventoryScreenHelper.InventoryCategoryType.Weapon: // 如果是武器类别
                    this._dataSource.ExecuteFilterWeapons(); // 执行武器过滤
                    return;
                case InventoryScreenHelper.InventoryCategoryType.Shield: // 如果是盾牌类别
                    break;
                case InventoryScreenHelper.InventoryCategoryType.HorseCategory: // 如果是马匹类别
                    this._dataSource.ExecuteFilterMounts(); // 执行坐骑过滤
                    return;
                case InventoryScreenHelper.InventoryCategoryType.Goods: // 如果是商品类别
        ...
        {
            SkillInventoryScreenHelper.Instance.CloseInventoryPresentation(false); // 关闭技能库存展示
        }
        ...
        private GauntletMovieIdentifier _gauntletMovie; // Gauntlet 电影对象
