...
namespace New_ZZZF
{
    public class SPSkillVM : ViewModel, IInventoryStateHandler
    {
        ...
        {
            this._usageType = SkillInventoryScreenHelper.Instance.CurrentMode;
        ...
        private ItemVM ProcessCompareItem(ItemVM item, int alternativeUsageIndex = 0)
        {
            ...
            if (!InventoryLogic.IsEquipmentSide(((SPSkillItemVM)item).InventorySide))
            {
                if (flag2)
                {
        ...
        private void ResetComparedItems(ItemVM item, int alternativeUsageIndex)
        {
            ...
            this.ItemMenu.SetItem(this._selectedItem, InventoryLogic.InventorySide.None, comparedItem, this._currentCharacter, alternativeUsageIndex);
        }
        ...
        public void ProcessItemTooltip(ItemVM item)
        {
            ...
            this.ItemMenu.SetItem(this._selectedItem, InventoryLogic.InventorySide.None, comparedItem, this._currentCharacter, 0);
        ...
        public void RefreshComparedItem()
        {
            ...
            {
                this.ItemMenu.SetItem(this._selectedItem, InventoryLogic.InventorySide.None, this._comparedItemList[this._lastComparedItemIndex], this._currentCharacter, 0);
            }
        }
        ...
        private void AfterTransfer(InventoryLogic inventoryLogic, List<TransferCommandResult> results)
        {
            ...
            for (int num = 0; num != results.Count; num++)
            {
                ...
                else if (InventoryLogic.IsEquipmentSide(transferCommandResult.ResultSide))
                {
                    ...
                    {
                        SPSkillItemVM3 = new SPSkillItemVM(this._inventoryLogic, this.MainCharacter.IsFemale, this.CanCharacterUseItemBasedOnSkills(transferCommandResult.EffectedItemRosterElement.EquipmentElement.Item.StringId), this._usageType, transferCommandResult.EffectedItemRosterElement, transferCommandResult.ResultSide, this._inventoryLogic.GetCostOfItemRosterElement(transferCommandResult.EffectedItemRosterElement, transferCommandResult.ResultSide), new EquipmentIndex?(transferCommandResult.EffectedEquipmentIndex));
                        SPSkillItemVM3.IsNew = true;
                    }
                    this.UpdateEquipment(this.ActiveEquipment, SPSkillItemVM3, transferCommandResult.EffectedEquipmentIndex);
        ...
        private void RefreshInformationValues()
        {
            ...
            // 获取玩家队伍的总库存容量
            int inventoryCapacity = (int)Campaign.Current.Models.InventoryCapacityModel.CalculateInventoryCapacity(MobileParty.MainParty, false, false, 0, 0, 0, false).ResultNumber;
        ...
        public bool IsItemEquipmentPossible(SPSkillItemVM itemVM)
        {
            ...
            switch (this.TargetEquipmentType)
            {
                case EquipmentIndex.Head:
                    if (skill.Type == SPSkillType.MainActive)
                    { CanUseFlag2 = true; }
                    break;
                case EquipmentIndex.Cape:
                    if (skill.Type == SPSkillType.SubActive)
                    { CanUseFlag2 = true; }
                    break;
                case EquipmentIndex.Body:
                    if (skill.Type == SPSkillType.Passive || skill.Type == SPSkillType.Passive_Spell)
                    { CanUseFlag2 = true; }
                    break;
                ...
                case EquipmentIndex.Weapon3:
                    if (skill.Type == SPSkillType.CombatArt_Spell || skill.Type == SPSkillType.Spell || skill.Type == SPSkillType.Passive_Spell)
                    { CanUseFlag2 = true; }
                    break;
                case EquipmentIndex.Gloves:
                    if (skill.Type == SPSkillType.CombatArt || skill.Type == SPSkillType.Spell_CombatArt)
                    { CanUseFlag2 = true; }
                    break;
                default: break;
            }
        ...
        private void EquipEquipment(SPSkillItemVM itemVM)
        {
            ...
            SPSkillItemVM SPSkillItemVM = new SPSkillItemVM();
            SPSkillItemVM.RefreshWith(itemVM, this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment);
        ...
        private void UnequipEquipment(SPSkillItemVM itemVM)
        {
            ...
            TransferCommand command = TransferCommand.Transfer(
                amount: 1,
                fromSide: this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment,
                toSide: InventoryLogic.InventorySide.PlayerInventory,
        ...
        protected void RefreshEquipment(SPSkillItemVM itemVM, EquipmentIndex itemType)
        {
            switch (itemType)
            {
                case EquipmentIndex.None:
                    return;
                case EquipmentIndex.WeaponItemBeginSlot:
                    this.CharacterWeapon1Slot.RefreshWith(itemVM, this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment);
                    return;
                case EquipmentIndex.Weapon1:
                    this.CharacterWeapon2Slot.RefreshWith(itemVM, this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment);
                    return;
                case EquipmentIndex.Weapon2:
                    this.CharacterWeapon3Slot.RefreshWith(itemVM, this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment);
                    return;
                case EquipmentIndex.Weapon3:
                    this.CharacterWeapon4Slot.RefreshWith(itemVM, this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment);
                    return;
                case EquipmentIndex.ExtraWeaponSlot:
                    this.CharacterBannerSlot.RefreshWith(itemVM, this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment);
                    return;
                case EquipmentIndex.NumAllWeaponSlots:
                    this.CharacterHelmSlot.RefreshWith(itemVM, this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment);
                    return;
                case EquipmentIndex.Body:
                    this.CharacterTorsoSlot.RefreshWith(itemVM, this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment);
                    return;
                case EquipmentIndex.Leg:
                    this.CharacterBootSlot.RefreshWith(itemVM, this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment);
                    return;
                case EquipmentIndex.Gloves:
                    this.CharacterGloveSlot.RefreshWith(itemVM, this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment);
                    return;
                case EquipmentIndex.Cape:
                    this.CharacterCloakSlot.RefreshWith(itemVM, this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment);
                    return;
                case EquipmentIndex.ArmorItemEndSlot:
                    this.CharacterMountSlot.RefreshWith(itemVM, this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment);
                    return;
                case EquipmentIndex.HorseHarness:
                    this.CharacterMountArmorSlot.RefreshWith(itemVM, this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment);
                    return;
                default:
                    return;
            }
            ...
            if (character.IsHero)
            {
                if (!character.HeroObject.CanHeroEquipmentBeChanged())
                {
                    ...
                    else
                    {
                        ...
                        bool? flag2 = (clan != null) ? new bool?(clan.Heroes.Contains(character.HeroObject)) : null;
                        bool flag3 = true;
        ...
        private void UpdateLeftCharacter()
        {
            ...
            if (this._inventoryLogic.LeftRosterName != null)
            {
                ...
                if (settlement != null && SkillInventoryScreenHelper.Instance.CurrentMode == InventoryScreenHelper.InventoryMode.Warehouse)
                {
        ...
        private SPSkillItemVM InitializeCharacterEquipmentSlot(ItemRosterElement itemRosterElement, EquipmentIndex equipmentIndex)
        {
            ...
            {
                SPSkillItemVM = new SPSkillItemVM(this._inventoryLogic, this.MainCharacter.IsFemale, this.CanCharacterUseItemBasedOnSkills(itemRosterElement.EquipmentElement.Item.StringId), this._usageType, itemRosterElement, this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment, this._inventoryLogic.GetCostOfItemRosterElement(itemRosterElement, this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment), new EquipmentIndex?(equipmentIndex));
            }
            else
            {
                SPSkillItemVM = new SPSkillItemVM();
                SPSkillItemVM.RefreshWith(null, this.IsInWarSet ? InventoryLogic.InventorySide.BattleEquipment : InventoryLogic.InventorySide.CivilianEquipment);
            }
            return SPSkillItemVM;
        }
        ...
        private void InitializeInventory()
        {
            ...
            for (int j = 0; j < num; j++)
            {
                if (j < array.Length)
                {
                    ...
                    SPSkillItemVM SPSkillItemVM = new SPSkillItemVM(this._inventoryLogic, this.MainCharacter.IsFemale, this.CanCharacterUseItemBasedOnSkills(itemRosterElement.EquipmentElement.Item.StringId), this._usageType, itemRosterElement, InventoryLogic.InventorySide.PlayerInventory, this._inventoryLogic.GetCostOfItemRosterElement(itemRosterElement, InventoryLogic.InventorySide.PlayerInventory), null);
                ...
                if (j < array2.Length)
                {
                    ...
                    SPSkillItemVM SPSkillItemVM2 = new SPSkillItemVM(this._inventoryLogic, this.MainCharacter.IsFemale, this.CanCharacterUseItemBasedOnSkills(itemRosterElement2.EquipmentElement.Item.StringId), this._usageType, itemRosterElement2, InventoryLogic.InventorySide.OtherInventory, this._inventoryLogic.GetCostOfItemRosterElement(itemRosterElement2, InventoryLogic.InventorySide.OtherInventory), null);
                    this.UpdateFilteredStatusOfItem(SPSkillItemVM2);
        ...
        private void BuyItem(SPSkillItemVM item)
        {
            ...
            TransferCommand command = TransferCommand.Transfer(TaleWorlds.Library.MathF.Min(this.TransactionCount, b), InventoryLogic.InventorySide.OtherInventory, InventoryLogic.InventorySide.PlayerInventory, item.ItemRosterElement, item.ItemType, this.TargetEquipmentType, this._currentCharacter);
            this._inventoryLogic.AddTransferCommand(command);
        ...
        private void SellItem(SPSkillItemVM item)
        {
            ...
            TransferCommand command = TransferCommand.Transfer(TaleWorlds.Library.MathF.Min(this.TransactionCount, b), inventorySide, InventoryLogic.InventorySide.OtherInventory, item.ItemRosterElement, item.ItemType, this.TargetEquipmentType, this._currentCharacter);
            this._inventoryLogic.AddTransferCommand(command);
        }
        ...
        private float GetCapacityBudget(MobileParty party, bool isBuy)
        {
            if (isBuy)
            {
                int? num = (party != null) ? new int?((int)Campaign.Current.Models.InventoryCapacityModel.CalculateInventoryCapacity(party, false, false, 0, 0, 0, false).ResultNumber) : null;
        ...
        private void TransferAll(bool isBuy)
        {
            ...
            for (int i = 0; i < mbbindingList.Count; i++)
            {
                ...
                if (SPSkillItemVM2 != null && !SPSkillItemVM2.IsFiltered && SPSkillItemVM2 != null && !SPSkillItemVM2.IsLocked && SPSkillItemVM2 != null && SPSkillItemVM2.IsTransferable)
                {
                        ...
                        if (flag2)
                        {
                            ...
                            if (num4 >= capacityBudget && list2.Count > 0)
                            {
                                ...
                                for (int j = 0; j < list2.Count; j++)
                                {
                                    ...
                                    TransferCommand item = TransferCommand.Transfer(SPSkillItemVM3.ItemRosterElement.Amount, fromSide, inventorySide, SPSkillItemVM3.ItemRosterElement, EquipmentIndex.None, EquipmentIndex.None, this._currentCharacter);
                                    list3.Add(item);
                                }
                    ...
                    if (num3 > 0)
                    {
                        TransferCommand item2 = TransferCommand.Transfer(num3, fromSide, inventorySide, SPSkillItemVM2.ItemRosterElement, EquipmentIndex.None, EquipmentIndex.None, this._currentCharacter);
                        list.Add(item2);
                    }
                }
            IL_29E:;
            ...
            if (num <= capacityBudget)
            {
                ...
                {
                    TransferCommand item3 = TransferCommand.Transfer(SPSkillItemVM4.ItemRosterElement.Amount, fromSide, inventorySide, SPSkillItemVM4.ItemRosterElement, EquipmentIndex.None, EquipmentIndex.None, this._currentCharacter);
                    list.Add(item3);
                }
            }
        ...
        {
            if (SkillInventoryScreenHelper.Instance.CurrentMode == InventoryScreenHelper.InventoryMode.Loot)
            {
                InformationManager.ShowInquiry(new InquiryData("", GameTexts.FindText("str_leaving_loot_behind", null).ToString(), true, true, GameTexts.FindText("str_yes", null).ToString(), GameTexts.FindText("str_no", null).ToString(), delegate ()
                {
                    if (!this._isFinalized)
                    {
                        ...
                        SkillInventoryScreenHelper.Instance.CloseInventoryPresentation(true);
                    }
        ...
        {
            if (SkillInventoryScreenHelper.Instance.CurrentMode == InventoryScreenHelper.InventoryMode.Loot && !this._inventoryLogic.IsThereAnyChanges() && this._inventoryLogic.GetElementsInInitialRoster(InventoryLogic.InventorySide.OtherInventory).Any<ItemRosterElement>())
            {
        ...
        private void HandleDone()
        {
            if (!this._isFinalized)
            {
                ...
                if (flag && !flag2)
                {
                    ...
                    {
                        SkillInventoryScreenHelper.Instance.CloseInventoryPresentation(false);
                    }, null, "", 0f, null, null, null), false, false);
                }
                else if (num2 > 0)
                {
        ...
        public void ExecuteTransferWithParameters(SPSkillItemVM item, int index, string targetTag)
        {
            ...
            else if (targetTag.StartsWith("Equipment"))
            {
                ...
                if (item.InventorySide == InventoryLogic.InventorySide.PlayerInventory || InventoryLogic.IsEquipmentSide(item.InventorySide))
                {
                    // 调用 ProcessEquipItem 方法处理装备逻辑。
                    this.ProcessEquipItem(item);
                }
        ...
        {
            bool flag = true;
            SPSkillType searchTarget = SPSkillType.MainActive;
            switch (this.ActiveFilterIndex)
            {
                case 1:
                    searchTarget = SPSkillType.MainActive;
                    _activeFilterIndex = SPInventoryVM.Filters.Weapons;
                    break;
                case 2:
                    searchTarget = SPSkillType.SubActive;
                    _activeFilterIndex = SPInventoryVM.Filters.ShieldsAndRanged;
                    break;
                case 3:
                    searchTarget = SPSkillType.Spell;
                    _activeFilterIndex = SPInventoryVM.Filters.Armors;
                    break;
                case 4:
                    searchTarget = SPSkillType.Passive;
                    _activeFilterIndex = SPInventoryVM.Filters.Mounts;
                    break;
                case 5:
                    searchTarget = SPSkillType.CombatArt;
            ...
            !(searchTarget == SPSkillType.Passive && val.Type == SPSkillType.Passive_Spell)&&
            !(searchTarget == SPSkillType.CombatArt && val.Type == SPSkillType.CombatArt_Spell)&&
            !(searchTarget == SPSkillType.Spell && val.Type == SPSkillType.Spell_CombatArt);
        ...
        public void FilterInventoryAtOpening(InventoryScreenHelper.InventoryCategoryType inventoryCategoryType)
        {
