...
namespace New_ZZZF
{
    internal class HouYueSheJi : SkillBase
    {
        public HouYueSheJi()
        {
            ...
            Type = SPSkillType.SubActive;    // 类型必须明确
            Cooldown = 0;             // 冷却时间（秒）
            ResourceCost = 0f;        // 消耗
        ...
        public class HouYueSheJiBuff : AgentBuff
        {
            ...
            {//差一点的表达效果：直接结束时瞬移，造成伤害
                ...
                agent.SetActionChannel(0, ActionIndexCache.Create("act_none"), true, (AnimFlags)999UL);
                agent.SetActionChannel(1, ActionIndexCache.Create("act_none"), true, (AnimFlags)999UL);
